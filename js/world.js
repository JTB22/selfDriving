class World {
  ...
  generate() {
    ...
    this.buidlings = this.#generateBuildings();
  ...
  #generateTrees() {
    const points = [
      ...
      ...this.buidlings.map((b) => b.points).flat(),
    ];
    ...
    while (tryCount < 100) {
      ...
      // check if it's too close to road or building
      let keep = true;
  ...
  #generateBuildings() {
    ...
    for (const seg of this.graph.segments) {
      tmpEnvelopes.push(
        new Envelope(
          seg,
          this.buildingWidth + this.roadWidth + this.spacing * 2,
          this.roadRoundness
        )
      );
    }
